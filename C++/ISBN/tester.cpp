/* OOP244.132 Assignment 3 Test Main
...
int pTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
    ...
    if(list.isRegistered(-1)) {
        testFailed("isRegistered(-1) did not return false", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("isRegistered(-1)", ++testNumber, totalPassed, totalTested);

    if(list.isRegistered(99955)) {
        testFailed("isRegistered(99955) did not return false", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("isRegistered(99955)", ++testNumber, totalPassed, totalTested);

    if(!list.isRegistered(0)) {
        testFailed("isRegistered(0) did not return true", ++testNumber, &ok,
         totalTested);
    } else
    ...
    if(list.isRegistered(-1, "0")) {
        testFailed("isRegistered(-1, 0) did not return false", ++testNumber, &ok,
         totalTested);
    } else
    ...
    if(!list.isRegistered(0, "10")) {
        testFailed("isRegistered(0, 10) did not return true", ++testNumber, &ok,
         totalTested);
    } else
    ...
    if(list.isRegistered(0, "95000000")) {
        ...
        testPassed("isRegistered(0, 95000000)", ++testNumber, totalPassed, totalTested);

    // minimum number of digits
    //
    if(list.minNoDigits(-1) != 0) {
        testFailed("minNoDigits(-1) did not return 0", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("minNoDigits(-1)", ++testNumber, totalPassed, totalTested);

    if(list.minNoDigits(0) != 2) {
        testFailed("minNoDigits(0) did not return 2", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("minNoDigits(0)", ++testNumber, totalPassed, totalTested);

    if(list.minNoDigits(92) != 1) {
        testFailed("minNoDigits(92) did not return 1", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("minNoDigits(92)", ++testNumber, totalPassed, totalTested);

    if(list.minNoDigits(99955) != 0) {
        testFailed("minNoDigits(99955) did not return 0", ++testNumber, &ok,
         totalTested);
    } else
...
int iTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
    ...
    isbn = ISBN();
    if(isbn.empty() == 0) {
        testFailed("empty() did not return true for ISBN()", ++testNumber, &ok,
         totalTested);
    } else
    ...
    if (strcmp(stra, strb)) {
        testFailed("toStr does not return same string for ISBN() and "
         "ISBN(\"9070002046\", )", ++testNumber, &ok, totalTested);
    } else
    ...
    if(strcmp(stra, "9070002043") != 0) {
        testFailed("toStr did not return 9070002043", ++testNumber, &ok,
         totalTested);
    } else
        testPassed("ISBN(9070002043) and toStr(char*)", ++testNumber, totalPassed,
         totalTested);
    ...
    if(!strcmp(strb, "9070002046")) {
        ...
        testPassed("ISBN(...) and toStr(char*)", ++testNumber, totalPassed, totalTested);
    ...
    if(strcmp(stra, "90-70002-04-3")) {
        testFailed("toStrWithStyle did not return 90-70002-04-3", ++testNumber, &ok,
         totalTested);
    } else
...
int oTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
    ...
    if(order.outstanding() != 0) {
        testFailed("Order(), outstanding() did not return 0", ++testNumber, &ok,
         totalTested);
    } else
    ...
    if(order.has(ISBN("9070002046", list)) == 0 || order.outstanding() != 0) {
        ...
        testPassed("has(\"9070002046\"), add(5), outstanding()", ++testNumber, totalPassed,
         totalTested);
   ...
   //
    ...
    if(order.has(ISBN("XXXXXXXXX0", list)) == 0 || order.outstanding() != 0) {
        ...
        testPassed("has(\"XXXXXXXXX0\"), add(5), outstanding()", ++testNumber, totalPassed,
         totalTested);
   ...
   //
    ...
    if(order.has(ISBN("014010867X", list)) == 0 || order.outstanding() != 0) {
        ...
        testPassed("has(\"014010867X\"), outstanding()", ++testNumber, totalPassed,
         totalTested);
    ...
    if(order.has(ISBN("014010867X", list)) == 0 || order.outstanding() != 5) {
        ...
        testPassed("has(\"014010867X\"), add(5), outstanding()", ++testNumber, totalPassed,
         totalTested);
      ...
      if (strcmp("XXXXXXXXX0", value[i]) == 0 ||
          strcmp("081613752X", value[i]) == 0) {
            if (isValid(value[i]) == 0 ){
              sprintf(message, "  isValid(...) returned "
                "false for %s", value[i]);
              testPassed(message, ++testNumber, totalPassed, totalTested);
            } else {
              sprintf(message, "  isValid(...) returned "
                "true   for %s", value[i]);
              testFailed(message, ++testNumber, &ok, totalTested);
            }
      // All other ISBN's are valid and registered
      } else if(!isbn.isRegistered()) {
            sprintf(message, "isRegistered() did not return true for %s",
             value[i]);
            testFailed(message, ++testNumber, &ok, totalTested);
        } else if (i%100 == 0) {
